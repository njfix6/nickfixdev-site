...
- **SPA fallback**: Two redundant mechanisms cover this — `wrangler.jsonc` sets `assets.not_found_handling: "single-page-application"` (used by the Workers static-assets deploy), and `public/_redirects` rewrites `/* → /index.html 200` (the older Pages convention, harmless to keep). Any new hosting target needs the equivalent rewrite.
